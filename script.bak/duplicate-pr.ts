...
import { createThinksoft } from "@thinksoft/sdk"
...
`)
  ...
  const Thinksoft = await createThinksoft({ port: 0 })
  ...
  try {
    ...
    const session = await Thinksoft.client.session.create()
    const result = await Thinksoft.client.session
      .prompt({
