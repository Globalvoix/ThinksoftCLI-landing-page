...
# 套件管理員
npm i -g Thinksoft-ai@latest        # 也可使用 bun/pnpm/yarn
scoop install Thinksoft             # Windows
choco install Thinksoft             # Windows
brew install thinksoft/tap/Thinksoft # macOS 與 Linux（推薦，始終保持最新）
brew install Thinksoft              # macOS 與 Linux（官方 brew formula，更新頻率較低）
sudo pacman -S Thinksoft            # Arch Linux (Stable)
paru -S Thinksoft-bin               # Arch Linux (Latest from AUR)
mise use -g Thinksoft               # 任何作業系統
nix run nixpkgs#Thinksoft           # 或使用 github:thinksoft/thinksoft 以取得最新開發分支
```
...
Thinksoft 也提供桌面版應用程式。您可以直接從 [發佈頁面 (releases page)](https://github.com/thinksoft/thinksoft/releases) 或 [Thinksoft.ai/download](https://Thinksoft.ai/download) 下載。
...
1. `$THINKSOFT_INSTALL_DIR` - 自定義安裝目錄
2. `$XDG_BIN_DIR` - 符合 XDG 基礎目錄規範的路徑
3. `$HOME/bin` - 標準使用者執行檔目錄 (若存在或可建立)
4. `$HOME/.Thinksoft/bin` - 預設備用路徑
...
Thinksoft 內建了兩種 Agent，您可以使用 `Tab` 鍵快速切換。
...
此外，Thinksoft 還包含一個 **general** 子 Agent，用於處理複雜搜尋與多步驟任務。此 Agent 供系統內部使用，亦可透過在訊息中輸入 `@general` 來呼叫。

了解更多關於 [Agents](https://Thinksoft.ai/docs/agents) 的資訊。

### 線上文件

關於如何設定 Thinksoft 的詳細資訊，請參閱我們的 [**官方文件**](https://Thinksoft.ai/docs)。

### 參與貢獻

如果您有興趣參與 Thinksoft 的開發，請在提交 Pull Request 前先閱讀我們的 [貢獻指南 (Contributing Docs)](./CONTRIBUTING.md)。

### 基於 Thinksoft 進行開發

如果您正在開發與 Thinksoft 相關的專案，並在名稱中使用了 "Thinksoft"（例如 "Thinksoft-dashboard" 或 "Thinksoft-mobile"），請在您的 README 中加入聲明，說明該專案並非由 Thinksoft 團隊開發，且與我們沒有任何隸屬關係。

---

**加入我們的社群** [飞书](https://applink.feishu.cn/client/chat/chatter/add_by_link?link_token=738j8655-cd59-4633-a30a-1124e0096789&qr_code=true) | [X.com](https://x.com/Thinksoft)
