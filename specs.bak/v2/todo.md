...
The Thinksoft server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@thinksoft/llm` tool loop after replacing its
...
- plugins get global "Thinksoft" instance like in that post i showed
- Thinksoft instance has stuff like `Thinksoft.session.prompt()` or
  `Thinksoft.tool.register({...})`
...
- decide whether to preserve deprecated `@thinksoft/llm` orchestration exports
