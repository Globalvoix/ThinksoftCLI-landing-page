...
Both `action` and `resource` use Thinksoft's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.Thinksoft` files is intentionally deferred until `.Thinksoft` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether Thinksoft permits an operation through its managed execution paths.
