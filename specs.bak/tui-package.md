...
Move the canonical Thinksoft terminal application from
`packages/Thinksoft/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/Thinksoft`,
`packages/cli`, or `@thinksoft/core`.

The SDK is the TUI's Thinksoft boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/Thinksoft`.
- Do not replace private `packages/Thinksoft` imports with `@thinksoft/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@thinksoft/tui`.
...
- Thinksoft domain data displayed by the TUI
...
```text
packages/Thinksoft/src/cli/cmd/tui
```

Its private dependency on `packages/Thinksoft` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/Thinksoft/src/*`.
...
Status: Completed. The private `@thinksoft/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@thinksoft/tui`.
...
- The package has no dependency on `Thinksoft`, `@thinksoft/cli`, or
  `@thinksoft/core`.
...
primitives now live in `@thinksoft/tui`. The legacy host consumes explicit
...
and build inputs through `@thinksoft/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Thinksoft globals or process state;
...
Replace process-global Thinksoft state with resolved TUI inputs.
...
`@thinksoft/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/Thinksoft` initially.
...
canonical in `@thinksoft/tui`. Configured references resolve through the new
...
adapters rather than Thinksoft domain implementations.

Make the SDK the only Thinksoft domain boundary used by the TUI.
...
- Domain-facing TUI code imports Thinksoft data and operations only from
  `@thinksoft/sdk`.
...
`@thinksoft/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/Thinksoft`.
...
- The package has no imports from `packages/Thinksoft`, `packages/cli`, or
  `@thinksoft/core`.
...
public `@thinksoft/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/Thinksoft`.
- Keep the legacy embedded worker and server startup in `packages/Thinksoft`.
- Change those adapters to load config, create transport inputs, and call the
  public `@thinksoft/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/Thinksoft`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@thinksoft/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@thinksoft/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Thinksoft domain state.
...
cd packages/Thinksoft && bun typecheck
...
rg '@thinksoft/core|packages/Thinksoft|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/Thinksoft/src packages/Thinksoft/test
```
...
- Build the current-platform `packages/Thinksoft` binary.
