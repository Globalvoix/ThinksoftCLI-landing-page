# Remove `packages/Thinksoft/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/Thinksoft/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the Thinksoft legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/Thinksoft/src/storage/db.ts` are concentrated in 22 source files:

- `packages/Thinksoft/src/account/repo.ts`
- `packages/Thinksoft/src/cli/cmd/db.ts`
- `packages/Thinksoft/src/cli/cmd/import.ts`
- `packages/Thinksoft/src/cli/cmd/stats.ts`
- `packages/Thinksoft/src/control-plane/workspace.ts`
- `packages/Thinksoft/src/index.ts`
- `packages/Thinksoft/src/node.ts`
- `packages/Thinksoft/src/permission/index.ts`
- `packages/Thinksoft/src/project/project.ts`
- `packages/Thinksoft/src/server/projectors.ts`
- `packages/Thinksoft/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/Thinksoft/src/server/shared/fence.ts`
- `packages/Thinksoft/src/session/message-v2.ts`
- `packages/Thinksoft/src/session/projectors.ts`
- `packages/Thinksoft/src/session/prompt.ts`
- `packages/Thinksoft/src/session/session.ts`
- `packages/Thinksoft/src/session/todo.ts`
- `packages/Thinksoft/src/share/share-next.ts`
- `packages/Thinksoft/src/storage/db.ts`
- `packages/Thinksoft/src/sync/index.ts`
- `packages/Thinksoft/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy Thinksoft database wrapper; `packages/Thinksoft/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the Thinksoft wrapper.
...
Status: Completed. `SyncEvent` and the Thinksoft projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy Thinksoft database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy Thinksoft database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy Thinksoft database wrapper.
...
All migration groups are complete or superseded. `packages/Thinksoft/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No Thinksoft data-migration group remains.

The previous Thinksoft `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate Thinksoft data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/Thinksoft`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/Thinksoft/src`
- `bun typecheck` from `packages/Thinksoft`
- Relevant package tests from `packages/Thinksoft`, not the repo root
