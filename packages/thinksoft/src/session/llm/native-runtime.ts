...
import type { LLMClientShape } from "@thinksoft/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("Thinksoft"))
    return { type: "unsupported", reason: "provider is not openai, Thinksoft, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @thinksoft/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains Thinksoft-owned. The native runtime only adapts
      // the @thinksoft/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
