...
import type { Message, ThinksoftClient, SessionMessageResponse } from "@thinksoft/sdk/v2"
...
export const AuthMethodID = "Thinksoft-login"
...
export class Service extends Context.Service<Service, Interface>()("@Thinksoft/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `Thinksoft auth login` in the terminal",
      name: "Login with Thinksoft",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: ThinksoftClient) {
...
function makeUsageService(sdk: ThinksoftClient) {
...
async function loadDirectorySnapshot(sdk: ThinksoftClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, Thinksoft provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const ThinksoftProvider = providers[ProviderV2.ID.make("Thinksoft")]
  const ThinksoftModel = ThinksoftProvider ? Provider.sort(Object.values(ThinksoftProvider.models))[0] : undefined
  if (ThinksoftProvider && ThinksoftModel) return { providerID: ThinksoftProvider.id, modelID: ThinksoftModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Thinksoft service failure", service })
}
