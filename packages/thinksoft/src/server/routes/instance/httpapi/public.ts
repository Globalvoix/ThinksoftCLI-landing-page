...
import { ThinksoftHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for Thinksoft serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(ThinksoftHttpApi)
...
export const PublicApi = ThinksoftHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "Thinksoft",
    version: "1.0.0",
    description: "Thinksoft api",
