...
export const DEPRECATED_PLUGIN_PACKAGES = ["Thinksoft-openai-codex-auth", "Thinksoft-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, ThinksoftVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(ThinksoftVersion) || semver.major(ThinksoftVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.Thinksoft
  if (typeof range !== "string") return
  if (!semver.satisfies(ThinksoftVersion, range)) {
    throw new Error(`Plugin requires Thinksoft ${range} but running ${ThinksoftVersion}`)
  }
}
