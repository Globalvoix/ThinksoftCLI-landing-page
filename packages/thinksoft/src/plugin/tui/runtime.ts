...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@thinksoft/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { errorData, errorMessage } from "@thinksoft/tui/util/error"
...
import { hasTheme, upsertTheme } from "@thinksoft/tui/context/theme"
import { Global } from "@thinksoft/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@thinksoft/core/util/flock"
import { Flag } from "@thinksoft/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@thinksoft/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@thinksoft/core/v1/config/plugin"
import { createCommandShim } from "@thinksoft/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@thinksoft/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".Thinksoft"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".Thinksoft", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".Thinksoft", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.THINKSOFT_PURE ? [] : pluginOrigins
    if (Flag.THINKSOFT_PURE && pluginOrigins.length) {
    }
