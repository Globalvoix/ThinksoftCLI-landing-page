...
import { isRecord } from "@thinksoft/tui/util/record"
...
import { TuiKeybind } from "@thinksoft/tui/config/keybind"
...
import { TuiConfig } from "@thinksoft/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@Thinksoft/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old Thinksoft.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.Thinksoft`
  // folders between cwd and home, and THINKSOFT_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.THINKSOFT_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit THINKSOFT_TUI_CONFIG override, if set.
  if (Flag.THINKSOFT_TUI_CONFIG) {
    const configFile = Flag.THINKSOFT_TUI_CONFIG
  ...
  // 4. `.Thinksoft` directories (and THINKSOFT_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".Thinksoft") || dir === Flag.THINKSOFT_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".Thinksoft") && dir !== Flag.THINKSOFT_CONFIG_DIR) continue
