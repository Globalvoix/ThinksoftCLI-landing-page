...
const TUI_SCHEMA_URL = "https://Thinksoft.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from Thinksoft.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const Thinksoft = await ThinksoftFiles(input)
  for (const file of Thinksoft) {
...
async function ThinksoftFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "Thinksoft"),
    ...(await Filesystem.findUp(["Thinksoft.json", "Thinksoft.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "Thinksoft"))
  }
  if (Flag.THINKSOFT_CONFIG) files.push(Flag.THINKSOFT_CONFIG)
