import type { PermissionV1 } from "@thinksoft/core/v1/permission"
// CLI entry point for `Thinksoft run`.
//
// Handles three modes:
...
//      Thinksoft server and runs interactive mode against it.
//
...
import { createThinksoftClient, type ThinksoftClient, type ToolPart } from "@thinksoft/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<ThinksoftClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run Thinksoft with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running Thinksoft server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to THINKSOFT_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to THINKSOFT_SERVER_USERNAME or 'Thinksoft')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: ThinksoftClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: ThinksoftClient, sessionID: string) {
      ...
      async function current(sdk: ThinksoftClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: ThinksoftClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: ThinksoftClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: ThinksoftClient) {
        ...
        async function loop(client: ThinksoftClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createThinksoftClient({
        baseUrl: "http://Thinksoft.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
