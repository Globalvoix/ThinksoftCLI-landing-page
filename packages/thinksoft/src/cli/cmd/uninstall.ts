...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall Thinksoft and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Thinksoft")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g Thinksoft-ai",
      pnpm: "pnpm uninstall -g Thinksoft-ai",
      bun: "bun remove -g Thinksoft-ai",
      yarn: "yarn global remove Thinksoft-ai",
      brew: "brew uninstall Thinksoft",
      choco: "choco uninstall Thinksoft",
      scoop: "scoop uninstall Thinksoft",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "Thinksoft", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using Thinksoft!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# Thinksoft") || content.includes(".Thinksoft/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".Thinksoft/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".Thinksoft/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".Thinksoft"))
    ) {
      continue
    }
