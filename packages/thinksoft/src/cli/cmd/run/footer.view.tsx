...
import { createColors, createFrames } from "@thinksoft/tui/ui/spinner"
import {
      ...
      <Show
        ...
        fallback={
          <box width="100%" flexDirection="column" gap={0}>
            <Show when={props.pet().visible()}>
              <box height={2} flexShrink={0} paddingLeft={1} paddingRight={1}
                   backgroundColor={panel() || prompt() ? runTheme().background : theme().surface}>
                <box flexDirection="column" gap={0}>
                  <text>{props.pet().catArt()[0]}</text>
                  <box flexDirection="row" gap={3}>
                    <text>{props.pet().catArt()[1]}</text>
                    <text fg={theme().muted} wrapMode="none" truncate flexGrow={1}>
                      {props.pet().message()}
                    </text>
                  </box>
                </box>
