...
import { normalizePromptContent } from "@thinksoft/tui/editor"
...
import { THINKSOFT_BASE_MODE, useBindings } from "@thinksoft/tui/keymap"
...
export function RunPromptBody(props: {
  ...
  return (
    <box width="100%" flexDirection="row" borderStyle="rounded" border={true} borderColor={props.theme().border}>
...
export function createPromptState(input: PromptInput): PromptState {
  ...
  const slashOptions = createMemo<SlashOption[]>(() => {
    const builtins = [
      ...
      { kind: "slash", name: "exit", display: "/exit", description: "close Thinksoft" } satisfies SlashOption,
    ]
