...
import type { ThinksoftClient, PermissionRequest, QuestionRequest, ToolPart } from "@thinksoft/sdk/v2"
import type { TuiConfig } from "@thinksoft/tui/config"
...
type PromptModel = Parameters<ThinksoftClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<ThinksoftClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<ThinksoftClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<ThinksoftClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<ThinksoftClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<ThinksoftClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<ThinksoftClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<ThinksoftClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<ThinksoftClient["question"]["reject"]>[0]
