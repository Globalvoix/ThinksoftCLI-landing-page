...
it.instance("plan agent denies edits except .Thinksoft/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".Thinksoft/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".Thinksoft", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.THINKSOFT_TEST_HOME
      process.env.THINKSOFT_TEST_HOME = test.directory
