...
import { SessionV1 } from "@thinksoft/core/v1/session"
import { FSUtil } from "@thinksoft/core/fs-util"
import { ModelsDev } from "@thinksoft/core/models-dev"
import { HttpRecorder } from "@thinksoft/http-recorder"
import { HttpRecorderInternal } from "@thinksoft/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@thinksoft/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@thinksoft/llm/route"
import { Env } from "@/env"
...
import { ProviderV2 } from "@thinksoft/core/provider"
import { ModelV2 } from "@thinksoft/core/model"

const FIXTURES_DIR = path.join(import.meta.dir, "../fixtures/recordings")

const zenURL = (connection: string) => `https://console.Thinksoft.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.THINKSOFT_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["Thinksoft", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("THINKSOFT_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("THINKSOFT_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["Thinksoft", "native", "oauth", "tool-loop"],
  ...
  {
    id: "Thinksoft-proxy",
    name: "Thinksoft proxy",
    providerID: ProviderV2.ID.Thinksoft,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["Thinksoft", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.THINKSOFT_RECORD_CONSOLE_TOKEN && process.env.THINKSOFT_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.Thinksoft,
        name: "Thinksoft Zen",
        env: ["THINKSOFT_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.THINKSOFT_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.THINKSOFT_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.THINKSOFT_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["Thinksoft", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("THINKSOFT_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("THINKSOFT_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("THINKSOFT_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set THINKSOFT_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the Thinksoft LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "Thinksoft.json"),
      JSON.stringify({ $schema: "https://Thinksoft.ai/config.json", ...scenario.config(model) }),
    ),
  )
