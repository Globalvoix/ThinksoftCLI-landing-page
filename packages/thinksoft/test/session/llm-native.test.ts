...
import { LLMEvent, ToolFailure } from "@thinksoft/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@thinksoft/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("selects native request routes for provider packages", () => {
    ...
    const compatible = LLMNative.model({
      model: {
        ...baseModel,
        providerID: ProviderV2.ID.make("Thinksoft"),
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("Thinksoft") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("Thinksoft") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
    ...
    ).toEqual({ type: "unsupported", reason: "provider is not openai, Thinksoft, or anthropic" })
    expect(
      LLMNativeRuntime.status({
        model: baseModel,
  ...
  test("prefers console provider api key over stored Thinksoft auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("Thinksoft") },
        provider: {
          ...providerInfo,
          id: ProviderV2.ID.make("Thinksoft"),
