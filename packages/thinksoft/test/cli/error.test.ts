...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/Thinksoft.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/Thinksoft.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/Thinksoft.jsonc", dir: ".Thinksoft", suggestion: "Thinksoft" },
        expected:
          'Directory ".Thinksoft" in /tmp/Thinksoft.jsonc is not valid. Rename the directory to "Thinksoft" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/Thinksoft.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/Thinksoft.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats account transport errors clearly", () => {
    const error = new AccountTransportError({
      method: "POST",
      url: "https://console.Thinksoft.ai/auth/device/code",
    })

    const formatted = FormatError(error)

    expect(formatted).toContain("Could not reach POST https://console.Thinksoft.ai/auth/device/code.")
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `Thinksoft models` to list available models",
      "Or check your config (Thinksoft.json) provider/model names",
    ].join("\n")
