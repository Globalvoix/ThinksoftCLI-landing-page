...
describe("Thinksoft read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // THINKSOFT_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ Thinksoft }) =>
      Effect.gen(function* () {
        const r = yield* Thinksoft.spawn(["mcp", "list"])
        Thinksoft.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ Thinksoft }) =>
      Effect.gen(function* () {
        const r = yield* Thinksoft.spawn(["providers", "list"])
        Thinksoft.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ Thinksoft }) =>
      Effect.gen(function* () {
        const r = yield* Thinksoft.spawn(["models"])
        Thinksoft.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ Thinksoft }) =>
      Effect.gen(function* () {
        const r = yield* Thinksoft.spawn(["agent", "list"])
        Thinksoft.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh THINKSOFT_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ Thinksoft }) =>
      Effect.gen(function* () {
        const r = yield* Thinksoft.spawn(["session", "list"])
        Thinksoft.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ Thinksoft }) =>
      Effect.gen(function* () {
        const r = yield* Thinksoft.spawn(["stats"])
        Thinksoft.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under THINKSOFT_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ Thinksoft }) =>
      Effect.gen(function* () {
        const r = yield* Thinksoft.spawn(["db", "path"])
        Thinksoft.expectExit(r, 0, "db path")
