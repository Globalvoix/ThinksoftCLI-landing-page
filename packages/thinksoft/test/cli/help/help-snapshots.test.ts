...
// terminal sizes. The default Thinksoft tui command is excluded —
// `Thinksoft --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `Thinksoft --help` prints today;
...
// top-level help on `--help` and exits 1; not a real Thinksoft command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("Thinksoft CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ Thinksoft }) =>
      Effect.gen(function* () {
        const topLevel = yield* Thinksoft.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* Thinksoft.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`Thinksoft ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`Thinksoft ${argv.join(" ")} --help`)
        }
