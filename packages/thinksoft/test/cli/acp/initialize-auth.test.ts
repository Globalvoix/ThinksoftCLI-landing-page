...
describe("Thinksoft acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ Thinksoft }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ Thinksoft }))
        ...
        expect(initialized.agentInfo?.name).toBe("Thinksoft")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ Thinksoft }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ Thinksoft })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("Thinksoft-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "Thinksoft-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.THINKSOFT_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ Thinksoft }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("Thinksoft-login")
