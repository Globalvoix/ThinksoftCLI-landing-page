...
import { ThinksoftClient, type GlobalEvent } from "@thinksoft/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<ThinksoftClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<ThinksoftClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<ThinksoftClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<ThinksoftClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<ThinksoftClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://Thinksoft.test"),
...
function sdk(
  input: {
    ...
    subscribe?: ThinksoftClient["event"]["subscribe"]
    globalEvent?: ThinksoftClient["global"]["event"]
    promptAsync?: ThinksoftClient["session"]["promptAsync"]
    status?: ThinksoftClient["session"]["status"]
    messages?: ThinksoftClient["session"]["messages"]
    children?: ThinksoftClient["session"]["children"]
    permissions?: ThinksoftClient["permission"]["list"]
    questions?: ThinksoftClient["question"]["list"]
  } = {},
) {
  const client = new ThinksoftClient()

  const subscribe: ThinksoftClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: ThinksoftClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: ThinksoftClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: ThinksoftClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: ThinksoftClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: ThinksoftClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: ThinksoftClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: ThinksoftClient["question"]["list"] = input.questions ?? (() => ok([]))
