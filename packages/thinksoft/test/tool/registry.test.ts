...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".Thinksoft")])),
})
...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .Thinksoft/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const Thinksoft = path.join(test.directory, ".Thinksoft")
      const tool = path.join(Thinksoft, "tool")
  ...
  it.instance("ignores non-tool exports in .Thinksoft/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".Thinksoft", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  // came in through `plugin.list()` — `oh-my-Thinksoft` was registering a tool
  ...
  it.instance("loads tools from .Thinksoft/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const Thinksoft = path.join(test.directory, ".Thinksoft")
      const tools = path.join(Thinksoft, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".Thinksoft", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const Thinksoft = path.join(test.directory, ".Thinksoft")
        const customTools = path.join(Thinksoft, "tools")
        const plugin = path.join(Thinksoft, "node_modules", "@Thinksoft-ai", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(Thinksoft, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@thinksoft/plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            [
              'import { tool } from "@thinksoft/plugin"',
              "export default tool({",
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".Thinksoft", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(Thinksoft, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@thinksoft/plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(Thinksoft, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(Thinksoft, "node_modules", "cowsay")
