...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "Thinksoft.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "Thinksoft.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "Thinksoft.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "Thinksoft.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["Thinksoft.json", "Thinksoft.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "Thinksoft.json"),
        path.join(tmp.path, "Thinksoft.jsonc"),
        path.join(project, "Thinksoft.json"),
        path.join(project, "Thinksoft.jsonc"),
      ])
    })
  })
