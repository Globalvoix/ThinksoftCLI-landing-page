    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".Thinksoft/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const ThinksoftModel = {
      ...openaiModel,
      providerID: "Thinksoft",
      api: {
        id: "Thinksoft-test",
        url: "https://api.Thinksoft.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, ThinksoftModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.Thinksoft?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.Thinksoft?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          Thinksoft: { itemId: "msg_Thinksoft" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              Thinksoft: { itemId: "msg_THINKSOFT_part" },
    ...
    expect(result[0].providerOptions?.Thinksoft?.itemId).toBe("msg_Thinksoft")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.Thinksoft?.itemId).toBe("msg_THINKSOFT_part")
