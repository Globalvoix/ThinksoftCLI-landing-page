...
import type { HostPluginApi } from "@thinksoft/tui/plugin/slots"
...
export function createTuiPluginApi(opts: Opts = {}): HostPluginApi {
  ...
  const own = createThinksoftClient({
  ...
  let selected = opts.theme?.selected ?? "Thinksoft"
  const set =
    opts.theme?.set ??
    ((name: string) => {
  ...
  return {
    ...
    attention: {
      ...
      soundboard: {
        ...
        current: () => opts.attention?.soundboard?.current?.() ?? "Thinksoft.default",
