// Subprocess test harness for the Thinksoft CLI. Spawns the real binary against
...
//
// Configuration flows through Thinksoft's built-in test affordances:
//   - THINKSOFT_CONFIG_CONTENT      : provider config inline, no files to find
//   - THINKSOFT_TEST_HOME           : pins os.homedir() → tmpdir
//   - THINKSOFT_DISABLE_PROJECT_CONFIG : skip walking up for Thinksoft.json
//   - THINKSOFT_PURE                : skip external plugin discovery + install
//   - THINKSOFT_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `Thinksoft.run` is fully wired. The shape supports adding more
// builders (`Thinksoft.serve(opts)`, `Thinksoft.acp(opts)`, `Thinksoft.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of ThinksoftCli.
import { test, type TestOptions } from "bun:test"
import { FSUtil } from "@thinksoft/core/fs-util"
import { AppProcess } from "@thinksoft/core/process"
...
const ThinksoftRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(ThinksoftRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    THINKSOFT_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    THINKSOFT_CONFIG_CONTENT: configJson,
    THINKSOFT_DISABLE_PROJECT_CONFIG: "1",
    THINKSOFT_PURE: "1",
    THINKSOFT_DISABLE_AUTOUPDATE: "1",
    THINKSOFT_DISABLE_AUTOCOMPACT: "1",
    THINKSOFT_DISABLE_MODELS_FETCH: "1",
    THINKSOFT_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `Thinksoft run`. New flags should
...
// `Thinksoft serve` is a long-lived process — it never exits on its own.
...
// `Thinksoft acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type ThinksoftCli = {
  // High-level: run a single prompt against the test model. Short-lived.
  readonly run: (message: string, opts?: RunOpts) => Effect.Effect<RunResult>
  // Spawn `Thinksoft serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `Thinksoft acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly Thinksoft: ThinksoftCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("Thinksoft.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const serve = Effect.fn("Thinksoft.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "Thinksoft server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `Thinksoft serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("Thinksoft.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const Thinksoft: ThinksoftCli = { run, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, Thinksoft })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `Thinksoft.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "Thinksoft") {
...
// every call site. Use this for any test that needs the Thinksoft CLI fixture.
//
...
// resources (e.g. `Thinksoft.serve`) without an extra `Effect.scoped` wrapper —
