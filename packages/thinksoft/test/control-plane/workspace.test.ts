...
import { Database } from "@thinksoft/core/database/database"
import { ProjectV2 } from "@thinksoft/core/project"
import { ProjectTable } from "@thinksoft/core/project/sql"
import { AbsolutePath } from "@thinksoft/core/schema"
import { Session as SessionNs } from "@/session/session"
import { SessionID } from "@/session/schema"
import { SessionTable } from "@thinksoft/core/session/sql"
import { EventSequenceTable } from "@thinksoft/core/event/sql"
...
import { WorkspaceV2 } from "@thinksoft/core/workspace"
import { WorkspaceTable } from "@thinksoft/core/control-plane/workspace.sql"
...
import { Ripgrep } from "@thinksoft/core/ripgrep"

const originalEnv = {
  THINKSOFT_AUTH_CONTENT: process.env.THINKSOFT_AUTH_CONTENT,
  THINKSOFT_EXPERIMENTAL_WORKSPACES: process.env.THINKSOFT_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.THINKSOFT_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@Thinksoft.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.THINKSOFT_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=Thinksoft-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.THINKSOFT_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.THINKSOFT_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.THINKSOFT_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=Thinksoft-test")
