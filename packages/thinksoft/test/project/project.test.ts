...
import { ProjectDirectories } from "@thinksoft/core/project/directories"
...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const ThinksoftFile = path.join(tmp, ".git", "Thinksoft")
      expect(yield* Effect.promise(() => Bun.file(ThinksoftFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "Thinksoft")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "Thinksoft")
      const wrongCache = path.join(parentDir, ".git", "Thinksoft")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "Thinksoft")
      const cacheB = path.join(bareB, "Thinksoft")
      const wrongCache = path.join(parentDir, ".git", "Thinksoft")
