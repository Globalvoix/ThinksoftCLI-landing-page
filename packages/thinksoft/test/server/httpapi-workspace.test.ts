...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-Thinksoft-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-Thinksoft-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://Thinksoft.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-Thinksoft-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-Thinksoft-workspace")
