...
Delete `packages/Thinksoft/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/Thinksoft/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/Thinksoft/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/Thinksoft`
- TUI plugin loader tests in `packages/Thinksoft` if runtime plugin API wiring changed
