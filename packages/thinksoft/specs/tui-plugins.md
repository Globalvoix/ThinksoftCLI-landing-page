...
- Author package entrypoint is `@thinksoft/plugin/tui`.
...
```json
{
  "$schema": "https://Thinksoft.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/Thinksoft-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@thinksoft/plugin/tui`.
- `@thinksoft/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@thinksoft/plugin/tui"
...
- The value is a semver range checked against the running Thinksoft version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.Thinksoft` is absent, no check is performed (backward compatible).
...
- `Thinksoft plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `Thinksoft plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.Thinksoft` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.Thinksoft`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.Thinksoft`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.Thinksoft` dir or `THINKSOFT_CONFIG_DIR`, Thinksoft installs `@thinksoft/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@thinksoft/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Thinksoft's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Thinksoft registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Thinksoft pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@thinksoft/plugin/tui`.
...
- `message` is required; `title` defaults to `"Thinksoft"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `Thinksoft.default`.
...
- Local plugins persist installed themes under the local `.Thinksoft/themes` area near the plugin config source.
...
- `--pure` / `THINKSOFT_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.Thinksoft/plugins/tui-smoke.tsx`
- Local vim plugin: `.Thinksoft/plugins/tui-vim.tsx`
- Local smoke config: `.Thinksoft/tui.json`
- Local smoke theme: `.Thinksoft/plugins/smoke-theme.json`
