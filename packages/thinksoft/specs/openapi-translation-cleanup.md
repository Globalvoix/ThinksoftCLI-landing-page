...
Trim `packages/Thinksoft/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/Thinksoft/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/Thinksoft`.
- `[x]` `bun typecheck` from `packages/Thinksoft`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/Thinksoft/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/Thinksoft`.
- `[x]` `bun dev generate > /tmp/Thinksoft-openapi.json` from `packages/Thinksoft`.
...
- Edit `packages/Thinksoft/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/Thinksoft-openapi.json` from `packages/Thinksoft`.
...
- Check source schemas in files like `packages/Thinksoft/src/session/schema.ts`, `packages/Thinksoft/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/Thinksoft/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/Thinksoft/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
