...
Practical reference for a future `packages/server` split after the Thinksoft
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/Thinksoft`.
...
Until enough shared service code lives outside `packages/Thinksoft`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/Thinksoft`

It should not import `packages/Thinksoft` services while `packages/Thinksoft`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on Thinksoft-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/Thinksoft` implementation details.
